...
- `make test-routing`, an opt-in harness that runs ten routing and finance-boundary cases against the real conversational model. Nothing pinned the half of the system that depends on `SYSTEM_INSTRUCTION` — the existing routing tests cover the deterministic keyword router and say so — which is how a 25% cut to the instruction once reached a green suite while putting prose in `reply` for a Bengali finance question. It asserts the invariant rather than the wording: a finance answer must leave `reply` empty, must fill `reply_template`, and that template must contain no digits, since every figure comes from the host. Measured on `gemma3n:e2b`: the shipped instruction scores 10/10 and a 48% cut scores 4/10, with all six finance languages leaking prose
...
- The system instruction now asks for short answers: one sentence for a greeting, two or three for an ordinary question, more only when the question needs it. Replies to a greeting fell from 96 to 62 characters, which is a third less for the device to speak aloud. Latency barely moved — 6.40s to 6.18s — because the reply text is not what dominates: the JSON envelope, the history prefill and constrained decoding put a floor of roughly three seconds under any answer on this hardware. Kept for the shorter speech rather than the speed. Validated at 10/10 on `make test-routing` across two runs; the first draft of the rule carried a worked English example and scored 7/10, with Bengali and Spanish answering in English, so the rule ships as prose with no example. The instruction is now 952 tokens, up from 758, which the installer's context budget already accommodates since the reply ceiling halved at the same time
- `llm_context_turns` in the development profile drops from six to four, matching both device profiles. It carried more history than the hardware for no recorded reason, which made the simulator slower than the device it stands in for and hid the window that actually ships. The window slides, so the turns after the cached instruction change on every request and are re-evaluated each time — history is paid for per question, not once
