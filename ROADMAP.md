...
- [x] Shorten the system instruction. Attempted and rejected on measurement. A 25% cut (760 to 573 prompt tokens) broke two of nine routing cases, including the Bengali finance case, which returned prose in `reply` instead of leaving it empty with a placeholder template - a finance-boundary violation in the device's default language. The full instruction scored 9/9 on the same cases. It is load-bearing, not padding. The saving was also smaller than the earlier estimate implied: 187 tokens is about 1.4s of Pi prefill, not the six seconds quoted for the whole prompt. Prefix caching, not prompt length, is what keeps this cost off the common path.
