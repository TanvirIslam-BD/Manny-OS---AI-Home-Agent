...
The Pi profile serves `gemma3n:e2b` from Ollama on loopback, hardened through a systemd drop-in (ADR-020). Manny asks it for a schema-constrained decision containing a BCP-47 language tag and either a same-language general reply or one approved finance intent. For finance, the model can return only a placeholder template; deterministic host code validates its exact fields and inserts MCP values afterward. Pydantic validates the response, allows one repair attempt, and falls back to deterministic behavior if the local model is unavailable. Only recent non-financial dialogue is kept in volatile memory.
