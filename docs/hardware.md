...
The default Pi inference budget is one 806 MB Q4_K_M language model, a 148 MB multilingual Whisper model, a 4,096-token runtime context, four CPU threads, one language-model inference slot, and a 6 GB systemd memory ceiling. These are safe starting values, not final performance claims; simultaneous STT, LLM, TTS, camera, latency, thermals, memory, and power must be measured on the actual Pi 5 enclosure.

Gemma 3 1B IT is the conversational default because the Pi 5 runs llama.cpp on four Cortex-A76 cores with no usable GPU offload, so token generation is bound by LPDDR4X bandwidth. The 4B variant exists for scene description, not for conversation. The local model never calls a tool: it returns a schema-constrained intent and wording, and the policy broker performs every MCP request, so 1B's job is classification rather than tool use.

`MANNY_GEMMA_VARIANT` (`1b`, `4b`) and `MANNY_GEMMA_QUANT` (`q4_k_m`, `q4_0`) select what `install_gemma_pi.sh` downloads. Only `1b` + `q4_k_m` has a checksum pinned in this repo; every other combination requires `MANNY_GEMMA_MODEL_URL` and `MANNY_GEMMA_MODEL_SHA256` that you verified yourself. `q4_0` generates and prefills faster on Cortex-A76, which has dotprod and so gets a blocked GEMM kernel that `Q4_K_M` does not; measure the routing accuracy cost before adopting it. The installer records its choice in `/opt/manny/model.env`, which `manny-llm.service` reads at start, and `MANNY_LLM_MODEL` in `/opt/manny/.env` must match the alias printed at the end of the install.
